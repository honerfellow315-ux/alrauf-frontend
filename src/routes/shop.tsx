...
export const Route = createFileRoute("/shop")({
  head: () => ({ meta: [{ title: "Shop — ALRauf Fragrances" }, { name: "description", content: "Browse our luxury fragrance collections." }] }),
  component: Shop,
});

const cats = [
  { id: "all", label: "All Fragrances" },
  { id: "wanted", label: "Wanted by Abdul Rauf" },
  { id: "flora", label: "Flora by Malka" },
] as const;
...
export function PageHeader({ title, sub }: { title: string; sub: string }) {
  return (
    <section className="relative py-20 sm:py-28 text-center overflow-hidden border-b border-gold/15">
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,oklch(0.78_0.14_82/0.12),transparent_60%)]" />
